"""
LEGACY MODULE - PRESERVED FOR REFERENCE

This module contains original utility functions from the initial RAG implementation.
Most functionality has been migrated to specialized modules for better organization:

RECOMMENDED ALTERNATIVES:
- embed_e5() & expand_query() → Use scripts/query_embed.py instead
- PDF extraction → Use scripts/pdf_cleaner.py (cleaner, more robust)
- Chunking → Use scripts/chunker.py (smart sentence splitting)
- Embedding & upsert → Use scripts/embed_and_upsert.py or scripts/embed_safe.py
- Full pipeline → Use scripts/ingest_all.py or scripts/pipeline.py

This file is kept for backward compatibility and reference but is NOT actively used
in the current pipeline. If you're starting new work, use the modules listed above.
"""
